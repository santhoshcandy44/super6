...
@HiltViewModel
class ImagePickerViewModel @Inject constructor(@ApplicationContext val context: Context, savedStateHandle: SavedStateHandle) :
    ViewModel() {


    private val args = savedStateHandle.toRoute<GalleyImagesPagerRoutes.GalleyImagesPager>()
    val MAX_ITEMS = args.maxItems


    private val _groupedByDateMediaItems =
        MutableStateFlow<Map<String, List<ImageMediaData>>>(emptyMap())
    ...
    private fun groupMediaFolders(medias: List<ImageMediaData>): Map<String, List<ImageMediaData>> {
    ...
    fun updateImageMediaIsSelected(mediaId: Long) {
        ...
        val currentGroupedData = _groupedByDateMediaItems.value

        // Create a new map to store the updated data
        val updatedGroupedData = currentGroupedData.mapValues { (_, mediaList) ->
            // Update the mediaList for each date
        ...
        _groupedByDateMediaItems.value = updatedGroupedData
    }


    fun updateAlbumMediaIsSelected(mediaId: Long) {
        // Get the current value of _groupedByDateMediaItems
        val currentGroupedData = _groupedByFolderMediaItems.value
        ...
        // Emit the updated grouped data to the MutableStateFlow
        _groupedByFolderMediaItems.value = updatedGroupedData
    }
