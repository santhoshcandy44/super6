        ...
        restore = {
            createNavController(context).apply {
                ...
                var allowedScreens = listOf(
                    ManageLocalJobRoutes.ManageLocalJob::class,
                    ManageLocalJobRoutes.CreateLocalJob::class
                )


                if(cleanedRoute == ManageLocalJobRoutes.ManagePublishedLocalJob::class.qualifiedName.orEmpty()){

                    if(!isSelectedUsedProductListingNull){
                        allowedScreens = allowedScreens.toMutableList()
                            .apply {

                                add(ManageLocalJobRoutes.ManagePublishedLocalJob::class)
                            }
                    }
